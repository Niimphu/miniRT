				...
				t_xyz intersection, t_intersect intersect)
{
	...
	if (intersects_sphere(intersection, *light->point,
		...
		|| intersects_cylinder(intersection, *light->point,
			scene->cylinders, light_direction) || intersects_cone(intersection, *light->point, scene->cone, light_direction))
		return (lighting);
			...
			t_rgb base, t_camera *camera)
{
	...
	specular_component = get_specular_component(lighting, intersect.material,
			intersect, camera);
